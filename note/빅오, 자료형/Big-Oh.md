...
>
>이때 빅오의 의미는 얼마만큼의 시간이 나오느냐에 대한 상한을 점근적으로 `n`에 대한  `다항식(polynomial)`으로 표시한 것이 됩니다.

>[!warning]
>`Big-Oh`가 나타내는 것과 `worst case`가 완전히 같은 것은 아닙니다. 해당 이유는 `Big-Oh`는 `n`이 매우 클 때의 전체적인 큰 그림에 집중하는 경향이 있기 때문입니다. 실세계에서 `n`이 얼마 되지 않을 경우, `Big-Oh`보다 더 높은 시간 복잡도가 요구되는 상황 또한 존재합니다.
>
>따라서 빅오 표기법은 **주어진 (최선/최악/평균) 경우의 수행 시간의 상한을 나타낸다**라고 이해하는 게 옳습니다. 
